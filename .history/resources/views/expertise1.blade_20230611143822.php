@extends('layout.master')


@section('dynblock')
<div class="about_section layout_padding">
    <div class="container">
        <div class="row">
            <h1 class="about_text">▎AIRCRAFT LEASING & PURCHASE</h1>
        </div>
        <div class="row">
            <div class="col-md-12">
                <p class="lorem_text">
                The Practice Group Aviation is responsible for, inter alia, the preparation, reviewing and execution of aircraft operating lease agreements (Wet and Dry Lease), the related conditions precedent documents and the issuance of legal opinions for various airlines in order to come to a closing on the lease of commercial aircrafts. Further, we liaise with legal counsels of lessors/lessees and the local aviation authority (SSCA) and provide legal advisory services relating to the local laws and regulations applicable to the aviation sector.</p>
            </div>
        </div>
        <div class="row">
                <div class="container">
                    <div class="col-6 col-sm-5 h-100">
                        <img class="img-fluid h-100" src="asset/images/Muoykheng.jpg" style="object-fit: cover;">
                    </div>
                <div class="col-6 col-sm-7 h-100 d-flex flex-column">
                    <div class="mt-auto p-4">
                        <h3>Doctor Name</h3>
                        <h6 class="fw-normal fst-italic text-primary mb-4">Cardiology Specialist</h6>
                        <p class="m-0">Dolor lorem eos dolor duo eirmod sea. Dolor sit magna rebum clita rebum dolor</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
  <!-- about section end -->
@stop
<div class="row">
    <div class="container">
        <div class="col-12 col-sm-6">
            <div class="card h-100">
                <img class="card-img-top" src="asset/images/Muoykheng.jpg" alt="Doctor Image" style="object-fit: cover;">
                <div class="card-body">
                    <h5 class="card-title">Doctor Name</h5>
                    <h6 class="card-subtitle mb-2 text-primary">Cardiology Specialist</h6>
                    <p class="card-text">Dolor lorem eos dolor duo eirmod sea. Dolor sit magna rebum clita rebum dolor</p>
                </div>
            </div>
        </div>
    </div>
</div>
